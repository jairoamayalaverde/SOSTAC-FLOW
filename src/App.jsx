<span className="text-xs text-slate-500 font-mono hidden md:block flex items-center gap-1">{session.user.email}</span>
                                ...
                                <div key={project.id} onClick={() => setSelectedProject(project)} className="group cursor-pointer rounded-xl p-6 transition-all border border-slate-800 bg-slate-900/40 hover:bg-slate-800 hover:border-amber-500/50 hover:shadow-lg relative overflow-hidden">
                                    ...
                                    <div className="flex justify-between items-start mb-4">
                                        <div>
                                            ...
                                            <div className="flex items-center gap-2 text-xs text-slate-400"><span>{project.client}</span><span className="w-1 h-1 bg-slate-600 rounded-full"></span><span className="text-amber-500 uppercase font-bold">{projectTemplates[project.project_type || project.projectType]?.name}</span></div>
                                        </div>
      ...
      <footer className="relative z-10 border-t border-slate-800 bg-slate-950/80 backdrop-blur-md pt-12 pb-12 mt-12">
        ...
        <div className="max-w-7xl mx-auto px-4 mt-12 pt-8 border-t border-slate-900 flex flex-col md:flex-row justify-between items-center text-xs text-slate-600"><p>© 2026 Jairo Amaya Full Stack Marketer. All rights reserved.</p><p className="font-mono">v10.0.0 CLOUD + ANALYTICS</p></div>
      </footer>
    </div>
  );

  // ========================================
  // RENDERIZADO CONDICIONAL: ANALYTICS
  // ========================================
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <div className="flex items-center gap-2 text-xs text-slate-500">
                ...
                <span className="text-amber-500 font-bold">{projectTemplates[selectedProject.project_type || selectedProject.projectType]?.name}</span>
              </div>
            </div>
          </div>
